<?php
class Grantee_bank_branch_model extends CI_Model {
	public function __construct() {
		$this->load->database();
		//////// ajax and ssp////////
		// Set table name
		$this->table = 'tbl_list_bank_branches';
	...
	public function add_grantee_bank_branch() {
		...
		if ($this->db->affected_rows() > 0) {
			...
			$get = $this->admin->getRecordById($this->input->post('tbl_banks_id'), $tbl_name = 'tbl_banks');
			...
			$this->logger
				...
				->detail(
					'<tr>' .
					'<td><strong>' . 'Bank Branch Name' . '</strong></td><td>' . ucwords($this->input->post('name') . '</td>')
					. '<td><strong>' . 'Branch Code' . '</strong></td><td>' . $this->input->post('branch_code') . '</td>' .
					'</tr>' .
					'<tr>'
					. '<td><strong>' . 'Bank Name' . '</strong></td><td>' . $get['name'] . '</td>' .
