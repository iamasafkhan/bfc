<?php
class Pay_scale_model extends CI_Model {
	public function __construct() {
		$this->load->database();
		//////// ajax and ssp////////
		// Set table name
		$this->table = 'tbl_pay_scale';
	...
	public function add_pay_scale() {
		...
		//insertion in db
		$this->db->insert('tbl_pay_scale', $data);
		if ($this->db->affected_rows() > 0) {
			...
			if ($this->db->affected_rows() > 0) {
				...
				$this->logger
					...
					->detail(
						'<tr>' .
						'<td><strong>' . 'Pay Scale Name' . '</strong></td><td>' . ucwords($this->input->post('name') . '</td>')
	...
	public function update_pay_scale() {
		...
		$result = $this->db->update('tbl_pay_scale', $data);
		if ($result == true) {
			// this is for activity log of a record
			if ($this->input->post('status') == '1') {$status = 'Active';} else { $status = 'Inactive';}
