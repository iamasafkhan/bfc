...
class Grants_model extends CI_Model {
	...
	public function add_grants() {
		...
		//insertion in db
		$this->db->insert('tbl_grants', $data);
		...
		if ($this->db->affected_rows() > 0) {
			...
			$this->logger
				...
				->detail(
					'<tr>' .
					'<td><strong>' . 'Grants Name' . '</strong></td><td>' . ucwords($this->input->post('name') . '</td>')
	...
	public function update_grants() {
		...
		$result = $this->db->update('tbl_grants', $data);
		if ($result == true) {
			// this is for activity log of a record
			if ($this->input->post('status') == '1') {$status = 'Active';} else { $status = 'Inactive';}
