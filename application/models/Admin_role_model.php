...
class Admin_role_model extends CI_Model {
	...
	public function add_admin_role() {
		...
		//insertion in db
		$this->db->insert('tbl_admin_role', $data);
		...
		if ($this->db->affected_rows() > 0) {
			...
			$this->logger
				...
				->detail(
					'<tr>' .
					'<td><strong>' . 'Role Name' . '</strong></td><td>' . ucwords($this->input->post('name') . '</td>')
	...
	public function update_admin_role() {
		...
		$result = $this->db->update('tbl_admin_role', $data);
		if ($result == true) {
			if ($this->input->post('status') == '1') {$status = 'Active';} else { $status = 'Inactive';}
			$this->logger
				->record_add_by($_SESSION['admin_id']) //Set UserID, who created this  Action
