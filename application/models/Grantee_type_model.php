<?php
class Grantee_type_model extends CI_Model {
	public function __construct() {
		$this->load->database();
		//////// ajax and ssp////////
		// Set table name
		$this->table = 'tbl_grantee_type';
	...
	public function add_grantee_type() {
		...
		//insertion in db
		$this->db->insert('tbl_grantee_type', $data);
		...
		if ($this->db->affected_rows() > 0) {
			...
			$this->logger
				...
				->detail(
					'<tr>' .
					'<td><strong>' . 'Grantee Type Name' . '</strong></td><td>' . ucwords($this->input->post('name') . '</td>')
	...
	public function update_grantee_type() {
		...
		$result = $this->db->update('tbl_grantee_type', $data);
		if ($result == true) {
			// this is for activity log of a record
			if ($this->input->post('status') == '1') {$status = 'Active';} else { $status = 'Inactive';}
